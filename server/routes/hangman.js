...
const WORD_CATEGORIES = {
  animals: [
    ...
    'cheetah', 'rhinoceros', 'octopus', 'butterfly', 'crocodile',
    'leopard', 'flamingo', 'peacock', 'hamster', 'squirrel'
  ],
  countries: [
    'australia', 'brazil', 'canada', 'denmark', 'egypt', 
    'france', 'germany', 'india', 'japan', 'mexico',
    'norway', 'portugal', 'sweden', 'thailand', 'vietnam'
  ],
  fruits: [
    'strawberry', 'pineapple', 'watermelon', 'blueberry', 'raspberry',
    'mango', 'orange', 'banana', 'coconut', 'papaya',
    'apricot', 'kiwi', 'lemon', 'cherry', 'peach'
  ],
  technology: [
    'computer', 'keyboard', 'software', 'internet', 'database',
    'algorithm', 'programming', 'smartphone', 'wireless', 'blockchain',
    'cybersecurity', 'artificial', 'network', 'server', 'laptop'
  ],
  sports: [
    'basketball', 'football', 'volleyball', 'cricket', 'baseball',
    'swimming', 'gymnastics', 'badminton', 'tennis', 'hockey',
    'archery', 'cycling', 'skiing', 'surfing', 'boxing'
  ],
  nature: [
    'mountain', 'rainbow', 'volcano', 'thunder', 'hurricane',
    'waterfall', 'glacier', 'desert', 'forest', 'ocean',
    'canyon', 'valley', 'river', 'island', 'cliff'
  ]
};
  ...
  try {
    ...
    if (!WORD_CATEGORIES[category]) {
      return res.status(400).json({ 
        error: 'Invalid category. Must be one of: animals, countries, fruits, technology, sports, nature' 
      });
    }
...
router.post('/validate', (req, res) => {
  try {
    ...
    const isComplete = wordLetters.every(letter => guessedLetters.includes(letter.toLowerCase()));
    const wrongGuesses = guessedLetters.filter(letter => !wordLetters.includes(letter));
