...
router.get("/challenge", (req, res) => {
  try {
    ...
    // Filter challenges by difficulty and select random one
    const challenges = wordChallenges.filter(
...
// Query params: difficulty
// returns array of challenges filtered by difficulty if provided
...
// Body: word, challengeId, availableLetters
router.post("/validate", (req, res) => {
  try {
    // Get word, challengeId, and availableLetters
    const { word, challengeId, availableLetters } = req.body;

    // Check if word, challengeId, and availableLetters are provided
    if (!word || !challengeId || !availableLetters) {
      return res
        .status(400)
        .json({
          error: "Word, challengeId, and availableLetters are required",
        });
    }
