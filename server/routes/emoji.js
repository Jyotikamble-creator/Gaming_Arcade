// Routes for emoji puzzles
// Includes starting a puzzle and fetching puzzles by difficulty
import express from "express";

// Create router
const router = express.Router();

// Each puzzle has id, emojis, answer, category, and difficulty
const puzzles = [
  // Nature & Environment
  {
    id: 1,
...
// Returns a random puzzle of the specified difficulty
// GET /api/emoji/difficulty/:level
router.get("/difficulty/:level", (req, res) => {
  ...
  // If no puzzles found, return error
  if (filteredPuzzles.length === 0) {
    return res
      .status(404)
      .json({ error: "No puzzles found for this difficulty" });
  }
  // Select random puzzle from filtered list
  const randomPuzzle =
    filteredPuzzles[Math.floor(Math.random() * filteredPuzzles.length)];
  // Log puzzle
  console.log("[EMOJI] Serving", level, "puzzle:", {
    id: randomPuzzle.id,
    answer: randomPuzzle.answer,
  });
  res.json(randomPuzzle);
