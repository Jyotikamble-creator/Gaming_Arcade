...
// Fetch a random word for word-guess from the route. Returns shape similar to older code: { data: { id, word, description } }
export const fetchRandomWord = async () => {
  try {
    ...
    const pick = rows.length
      ...
      : { id: null, word: "APPLE", description: "A fruit" };
    logger.debug(
      "Random word fetched",
      { word: pick.word },
      LogTags.WORD_GUESS
...
// fetch a random word for word-guess from the route. Generic stubs for other games - they will call conventional endpoints if implemented on the server
...
// Fetch math questions for math quiz from the route
export const fetchMathQuestions = () => {
  try {
    ...
    logger.error(
      "Failed to fetch math questions",
      error,
      {},
      LogTags.MATH_QUIZ
    );
...
// Fetch typing passage for typing test from route
export const fetchTypingPassage = () => {
  try {
    ...
    logger.error(
      "Failed to fetch typing passage",
      error,
      {},
      LogTags.TYPING_TEST
    );
...
// Fetch scramble words for word scramble from the route
export const fetchScramble = () => {
  try {
    ...
    logger.error(
      "Failed to fetch scramble words",
      error,
      {},
      LogTags.WORD_SCRAMBLE
    );
    throw error;
  }
};

// Fetch quiz questions  from the route
...
// Fetch emoji puzzle for emoji guess from the route
...
// Start whack-a-mole game from the route
export const startWhack = () => {
  try {
    ...
    logger.error(
      "Failed to start whack-a-mole game",
      error,
      {},
      LogTags.WHACK_MOLE
    );
...
// Start simon says game from the route
export const startSimon = () => {
  try {
    ...
    logger.error(
      "Failed to start simon says game",
      error,
      {},
      LogTags.SIMON_SAYS
    );
...
// Get leaderboard for a game from the route
