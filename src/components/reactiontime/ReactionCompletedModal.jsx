    ...
    <div className="fixed inset-0 bg-black/70 backdrop-blur-sm flex items-center justify-center z-50 p-4">
      <div className="bg-gradient-to-br from-gray-800 via-indigo-900 to-gray-800 rounded-3xl p-8 max-w-md w-full border-2 border-indigo-500/50 shadow-2xl animate-scale-in">
        ...
        <div className="bg-gray-900/50 rounded-2xl p-6 mb-6 space-y-4">
          ...
          <div className="flex justify-between items-center">
            ...
            <span className="text-2xl font-bold text-purple-400">
              {Math.round((1 - (Math.max(...reactionTimes) - Math.min(...reactionTimes)) / averageTime) * 100)}%
            </span>
          </div>
        </div>
        ...
        <div className="flex gap-4">
          <button
            onClick={onPlayAgain}
            className="flex-1 bg-gradient-to-r from-indigo-600 to-purple-600 hover:from-indigo-700 hover:to-purple-700 text-white font-bold py-4 px-6 rounded-xl transition-all duration-200 shadow-lg hover:scale-105"
          >
            Test Again
          </button>
