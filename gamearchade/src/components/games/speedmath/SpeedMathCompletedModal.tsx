...
const SpeedMathCompletedModal: React.FC<Props> = ({
  score,
  problemsSolved,
  totalProblems,
  bestStreak,
  ...
  const attempted = Math.max(totalProblems, 0);
  const correct = Math.max(problemsSolved, 0);
  ...
  const getDifficultyBadge = () => {
    switch (difficulty) {
      case 'easy':
        return { emoji: '🟢', label: 'Easy', color: 'text-green-400' };
      case 'medium':
        return { emoji: '🟡', label: 'Medium', color: 'text-yellow-400' };
      case 'hard':
        return { emoji: '🔴', label: 'Hard', color: 'text-red-400' };
      default:
    ...
    <div className="fixed inset-0 bg-black/70 backdrop-blur-sm flex items-center justify-center z-50 p-4">
      <div className="bg-linear-to-br from-gray-800 to-gray-900 rounded-2xl p-8 max-w-md w-full border-2 border-purple-500/50 shadow-2xl">
        ...
        <div className="grid grid-cols-2 gap-4 mb-6">
          ...
          <div className="bg-gray-700/50 rounded-lg p-4 text-center">
            <div className="text-gray-400 text-xs mb-1">Wrong</div>
            <div className="text-2xl font-bold text-white">{wrong}</div>
          </div>
