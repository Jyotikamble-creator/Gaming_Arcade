...
import { Trophy, Clock, RotateCcw, Home, Code } from 'lucide-react';
...
export default function PuzzleCompletedModal({
  ...
  const getPerformanceMessage = () => {
    if (score >= 1500) return { text: '🏆 Genius!', color: 'text-yellow-400' };
    if (score >= 1200) return { text: '⭐ Excellent!', color: 'text-blue-400' };
    if (score >= 900) return { text: '👍 Great Job!', color: 'text-green-400' };
    if (score >= 600) return { text: '🎯 Good!', color: 'text-orange-400' };
    ...
    <div className="fixed inset-0 bg-black/50 backdrop-blur-sm flex items-center justify-center z-50 p-4">
      ...
      >
        ...
        <div className="text-center mb-6">
          ...
          <h2 className="text-2xl font-bold text-light-text mb-2">Puzzle Solved!</h2>
          <div className={`text-xl font-bold ${performance.color}`}>
            {performance.text}
          </div>
          <p className="text-subtle-text text-sm mt-2 capitalize">{category} Challenge</p>
        </div>
