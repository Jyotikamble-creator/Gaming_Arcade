    ...
    <div className="container mx-auto px-4 py-16">
      ...
      <div className="text-center mb-16">
        <h1 className="text-5xl font-bold text-light-text mb-4">
          Welcome to <span className="text-primary-blue">Game Arcade</span>
        </h1>
      ...
      <div className="mb-16">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
          {games.map((game, index) => (
            <div 
              key={index}
              className="bg-card-bg p-6 rounded-lg shadow-xl text-center hover:shadow-2xl transition duration-300"
            >
              <div className="text-4xl mb-4">{game.icon}</div>
              <h3 className="text-xl font-semibold text-light-text mb-2">{game.title}</h3>
              <p className="text-subtle-text">{game.description}</p>
            </div>
          ))}
        </div>
      </div>
